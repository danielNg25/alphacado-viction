...
contract ZapperUniswapV3 is IERC721Receiver {
    ...
    function zapIn(
        ...
        require(
            isZapFromToken0 || token1 == order.fromToken,
            "Input token not present in liquidity pool"
        );
        ...
        if (dustBalance * token0Price > 1e18) {
            ...
            (uint128 addedLiquidity, , ) = nonfungiblePositionManager
    ...
    function zapOut(
        ...
        require(
            isZapToToken0 || token1 == order.toToken,
            "Output token not present in liquidity pool"
        );
        ...
        )
    {
        ...
        token0Price = FullMath.mulDiv(
            sqrtPriceX96.mul(1e18),
            sqrtPriceX96,
            2 ** 192
        );
    ...
    function calcMinToAmountForZapIn(
        ...
        require(
            isZapFromToken0 || pool.token1() == fromToken,
            "Input token not present in liquidity pool"
        );
    ...
    function calcMinToAmountForZapOut(
        ...
        require(
            isZapToToken0 || pool.token1() == toToken,
            "Output token not present in liquidity pool"
        );
