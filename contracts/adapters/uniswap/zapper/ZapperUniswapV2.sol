...
contract ZapperUniswapV2 is Zapper_ERC20_ERC20 {
    ...
    function calcMinToAmountForZapIn(
        ...
        require(
            asset0.token == fromToken || asset1.token == fromToken,
        ...
        if (asset0.token == fromToken) {
            uint swapAmount = _getAmountToSwap(
                router,
                pair,
                amount,
                asset0,
        ...
        return
            Math.min(
                amount0.mul(lpTotalSupply) / reserve0,
                amount1.mul(lpTotalSupply) / reserve1
            );
    }
    ...
    function calcMinToAmountForZapOut(
        ...
        require(
            isZapToToken0 || pair.token1() == toToken,
            "Input token not present in liquidity pair"
        );
    ...
    function _removeLiquidityAndSwap(
        ...
        address fromToken = token1 == toToken ? token0 : token1;
    ...
    function _balanceAssets(
        ...
        amounts[1] = _swap(router, swapAmountIn, tokenA, tokenB, extra);
    }
    ...
    function _getAmountOut(
        ...
        return
            router.getAmountOut(amountIn, assetIn.reserves, assetOut.reserves);
    }
