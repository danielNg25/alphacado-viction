...
error TargetChainAndExecutionParamsVersionMismatch(
    uint16 targetChain,
    uint8 version
);
...
error TargetChainAndExecutionInfoVersionMismatch(
    uint16 targetChain,
    uint8 version
);
...
function encodeEvmExecutionParamsV1(
    ...
    return
        abi.encode(
            uint8(ExecutionParamsVersion.EVM_V1),
...
function decodeEvmExecutionParamsV1(
    ...
    if (version != uint8(ExecutionParamsVersion.EVM_V1)) {
        revert UnexpectedExecutionParamsVersion(
            version,
            uint8(ExecutionParamsVersion.EVM_V1)
        );
    }
